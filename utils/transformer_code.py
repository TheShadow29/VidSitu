...
class Encoder(nn.Module):
    def __init__(self, d_model, d_hidden, n_vocab, n_layers, n_heads, drop_ratio, pe):
...
class RelMultiHead(nn.Module):
    ...
    def forward(self, query, key, value, pe=None):
        ...
        return self.wo(
            torch.cat(
                [
                    self.attention(q, k, v, pk, pv)
                    for q, k, v, pk, pv in zip(query, key, value, pe_k, pe_v)
                ],
                -1,
            )
        )
...
class RelEncoderLayer(nn.Module):
    def __init__(self, d_model, d_hidden, n_heads, drop_ratio, d_pe=None, sa=True):
        super().__init__()
        self.selfattn = ResidualBlock(
    ...
    def forward(self, x, pe=None):
        if not isinstance(x, dict):
            ...
            assert "key" in x
            assert "value" in x
            return self.feedforward(self.selfattn(x["query"], x["key"], x["value"], pe))
...
class RelEncoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                RelEncoderLayer(
                    d_model, d_hidden, n_heads, drop_ratio, d_pe=d_pe, sa=sa
                )
                for i in range(n_layers)
            ]
        )
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            d_model, d_hidden, n_vocab_src, n_layers, n_heads, drop_ratio, pe
        )
...
class RelTransformer(nn.Module):
    def __init__(
        ...
        self.encoder = RelEncoder(
            d_model, d_hidden, n_vocab_src, n_layers, n_heads, drop_ratio, pe, d_pe=d_pe
        )
