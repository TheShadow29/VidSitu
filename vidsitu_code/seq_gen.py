...
EncoderOut = NamedTuple(
    ...
    [
        ...
        # ("extra_proj", Optional[Tensor]),  # B x C
    ],
)


class SeqGenCustom(nn.Module):
    def __init__(
        self,
        models,
        tgt_dict,
    ...
    def _generate(
        ...
        enc_inp = {k: v for k, v in sample.items() if "prev_tok" not in k}
        encoder_outs = self.model.forward_encoder(enc_inp)
    ...
    def transpose_list(self, lst: List[List[int]]):
        # GeneratorExp aren't supported in TS so ignoring the lint
        min_len = min([len(x) for x in lst])  # noqa
        l2 = [[row[i] for row in lst] for i in range(min_len)]
        return l2
...
class EnsembleModel(nn.Module):
    ...
    def has_encoder(self):
        return self.single_model.use_encoder
        # return hasattr(self.single_model, "forward_encoder")
    ...
    def reorder_encoder_out(self, encoder_outs: Optional[List[EncoderOut]], new_order):
        ...
        for i, model in enumerate(self.models):
            assert encoder_outs is not None
            if hasattr(model, "reorder_encoder_out"):
